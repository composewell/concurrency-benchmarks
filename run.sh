...
print_help () {
  echo "Usage: $0 [--append] [--no-graphs] [--no-measure] [--use-fusion-plugin] -- <gauge options>"
  echo
...
do
  case $1 in
    ...
    --use-fusion-plugin)
        STACK_OPTS="$STACK_OPTS --flag concurrency-benchmarks:fusion-plugin";
        shift ;;
    --) shift; break ;;
    -*|--*) print_help ;;
    *) break ;;
...
$STACK build --bench --no-run-benchmarks $STACK_OPTS || die "build failed"
